...
export function AppleMapKitMap({ markers, directions }: AppleMapKitMapProps) {
  ...
  useEffect(() => {
    ...
    // Check if MapKit is already initialized to prevent "namespace already exists" error
    if (!window.mapkit.isInitialized) {
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes('namespace already exists')) {
          console.log("[MapKit] Already initialized, continuing...");
        } else {
    ...
    if (!mapInstance.current) {
      ...
      const center = new window.mapkit.Coordinate(markers[0].coordinate.lat, markers[0].coordinate.lng);
  ...
  useEffect(() => {
    ...
    markers.forEach((m) => {
      ...
      const annotation = new window.mapkit.MarkerAnnotation(
        new window.mapkit.Coordinate(m.coordinate.lat, m.coordinate.lng),
        markerOptions
      );
