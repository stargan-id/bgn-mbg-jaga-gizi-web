...
export function SidebarNavigation({ 
  ...
  const DesktopSidebarContent = () => (
    <TooltipProvider>
      <div className="flex flex-col h-full">
        {/* Header */}
        <div className={cn("border-b text-white relative", headerColor, isMinimized ? "p-3" : "p-6")}>
          {!isMinimized ? (
            <>
              <h2 className="text-xl font-bold">{title}</h2>
              {subtitle && (
          ...
          <Button
            ...
            className={cn(
              ...
              isMinimized ? "top-2 right-2" : "top-3 right-3"
            )}
          >
            {isMinimized ? (
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive 
                    ? 'bg-green-100 text-green-700 border border-green-200' 
                    : 'text-gray-600 hover:bg-gray-100',
                  isMinimized ? 'gap-0 px-2 py-3 justify-center' : 'gap-3 px-3 py-2.5'
                )}
              >
                <Icon className={cn("flex-shrink-0", isMinimized ? "w-5 h-5" : "w-5 h-5")} />
                {!isMinimized && (
                  <>
                    <span className="flex-1">{item.label}</span>
                    {item.badge && (
